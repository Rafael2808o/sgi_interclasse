...
export default function Login() {
    ...
    useEffect(() => {
        ...
        const dadosLoginRaw = localStorage.getItem("DadosLogin");
        if (dadosLoginRaw) {
            try {
                const dadosLogin = JSON.parse(dadosLoginRaw);
                if (dadosLogin.email) setEmail(dadosLogin.email);
                if (dadosLogin.senha) setSenha(dadosLogin.senha);
                setLembrar(true);
            } catch (e) {
            }
        }
        ...
        try {
            ...
            if (lembrar) {
                localStorage.setItem(
                    "Usuario Logado",
                    JSON.stringify({ token, usuario, lembrar: true })
                );
                localStorage.setItem("DadosLogin", JSON.stringify({ email, senha }));
                navigate("/principal");
} else {
                    ...
                    >
                        <PanelCard>
                            <form onSubmit={handleLogin}>
                                ...
                                <p style={{ color: "#ff6b6b", textAlign: "center", marginTop: "12px" }}>
                                    {mensagem}
                                </p>

                                <CreateAccountRow>
                                    <span>Ainda não tem uma conta?</span>
                                    <CreateAccountLink type="button">Criar conta</CreateAccountLink>
                                </CreateAccountRow>
                            </form>
                        </PanelCard>
